...
| ----------------------- | ------------------------------------------------------------ | --------------------------------------------------------------------------- |
| **Number Verification** | Verify the phone number of the connected drone device        | Pre-flight authentication to confirm drone device identity before takeoff   |
| **SIM Swap**            | Detect if SIM card has been swapped recently                 | Security check to prevent drone hijacking via SIM swap fraud                |
| **Device Swap**         | Detect if phone number's SIM has moved to a different device | Equipment integrity validation to ensure correct drone hardware is deployed |
